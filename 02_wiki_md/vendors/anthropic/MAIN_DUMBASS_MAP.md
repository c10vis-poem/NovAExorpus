---
source: MAIN_DUMBASS_MAP.md
type: markdown
cleaned: 2026-09-10
cleaner: tools/clean.py (mutool)
---

﻿Exact Repository Directory Structure Live on Drive


__NovÆxorpus_LIVING_MASTER_CANON/
├── 📄 README.md
├── 📄 MAP.md
├── 📄 00_DEFINITIVE_MASTER_SPECIFICATION_V3_COMPLETE.md
├── 📄 01_SOVEREIGN_NODE_AND_APK_TOPOLOGY.md
├── 📄 02_DUMBASS_UNIVERSAL_MEMORY_SPEC.md
├── 📄 03_DUAL_OPERATIONAL_HARNESS_AND_MCP_SPEC.md
├── 📄 04_ON_DEVICE_INGESTION_AND_W5H_FRAMEWORK.md
├── 📄 05_FEDERATED_FILE_TREE_TOPOLOGY_MASTER.md
├── 📄 RESUME.md
├── 📁 _dumbass_universal_memory/
│   ├── 📁 sqlite/
│   ├── 📁 postgres/
│   ├── 📁 mem0/
│   ├── 📁 ob1_protocol/
│   ├── 📁 omniroute/
│   ├── 📁 reasoning_bank/
│   ├── 📁 continual_harness/
│   ├── 📄 MAP.md
│   ├── 📄 manifest.jsonl
│   └── 📄 README.md
├── 📁 aesop-xi/
│   ├── 📁 arbitration/
│   ├── 📁 policies/
│   ├── 📁 .incognito_red_sandbox/
│   ├── 📄 MAP.md
│   ├── 📄 manifest.jsonl
│   ├── 📄 AGENTS.md
│   ├── 📄 RESUME.md
│   ├── 📄 UNRESOLVED.md
│   └── 📄 README_AESOP_XI.md
├── 📁 novus-aexenti/
│   ├── 📁 dual_agent_router/
│   ├── 📁 mem0_episodic/
│   ├── 📁 reasoning_bank/
│   ├── 📄 MAP.md
│   ├── 📄 manifest.jsonl
│   ├── 📄 AGENTS.md
│   ├── 📄 RESUME.md
│   ├── 📄 UNRESOLVED.md
│   └── 📄 README_NOVUS_AEXENTI.md
├── 📁 novaexopia/
│   ├── 📁 horizons-ui/
│   ├── 📁 aesc/
│   ├── 📁 aeyre/
│   ├── 📁 openwiki-tui-harness/
│   ├── 📁 modular_harnesses/
│   ├── 📁 mcp_connectors/
│   ├── 📄 MAP.md
│   ├── 📄 manifest.jsonl
│   ├── 📄 AGENTS.md
│   ├── 📄 RESUME.md
│   ├── 📄 UNRESOLVED.md
│   └── 📄 README_NOVAEXOPIA.md
├── 📁 skills-and-capabilities/
│   ├── 📁 code-review-graph/
│   ├── 📁 notebook-lmpy/
│   ├── 📁 obsidian-skills/
│   ├── 📁 prompt_skills/
│   │   └── 🛠️ (5 Matt Pocock workflow skills)
│   ├── 📄 MAP.md
│   ├── 📄 manifest.jsonl
│   ├── 📄 AGENTS.md
│   ├── 📄 RESUME.md
│   ├── 📄 UNRESOLVED.md
│   └── 📄 README_SKILLS_AND_CAPABILITIES.md
└── 📁 reviews_and_audits/
    ├── 📄 REVIEW-CANON-AND-BUILDERS-GUIDE.md
    ├── 📄 00_LEX_NOVI_MASTER_AUDIT_SYNTHESIS.md
    ├── 📄 AUDIT_LEX_NOVI_AESOP_XI.md
    ├── 📄 AUDIT_LEX_NOVI_AGENT_ASSETS.md
    └── 📄 AUDIT-03-BUILDERS-GUIDE.md

